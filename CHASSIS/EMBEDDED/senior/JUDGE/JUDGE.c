...
//			
				...
				case GAME_ROBOT_STATE_ID: // 比赛机器人状态：0x0201。发送频率：10Hz
					...
					break;
				case POWER_HEAT_DATA_ID: // 实时功率热量数据：0x0202。发送频率：10Hz
				{
						memcpy(&judge_rece_mesg.power_heat_data, data_addr, data_length);
					    remain_heat = 	judge_rece_mesg.game_robot_state.shooter_barrel_heat_limit-	judge_rece_mesg.power_heat_data.shooter_id1_17mm_cooling_heat;	
				...
				case SHOOT_DATA_ID: // 实时射击信息：0x0207。发送频率：射击后发送
				{
					   ...
					   if(judge_rece_mesg.shoot_data.bullet_speed < 15)
                        {
                            First_Order_Kalman_Filter_Cal(&Bullet_Speed_Kalman, 23.5);
							
                        }
                        else
                        {
                            First_Order_Kalman_Filter_Cal(&Bullet_Speed_Kalman, judge_rece_mesg.shoot_data.bullet_speed);
                        }
...
float First_Order_Kalman_Filter_Cal(First_Order_Kalman_Filter_t *_First_Order_Kalman_Filter, float _Z /*测量值*/)
{
    ...
    _First_Order_Kalman_Filter->Kalman_Gain =
        _First_Order_Kalman_Filter->Error_Est_Last / (_First_Order_Kalman_Filter->Error_Est_Last + _First_Order_Kalman_Filter->Error_Mea);
    // Step2:计算预测值
    _First_Order_Kalman_Filter->X_hat =
        _First_Order_Kalman_Filter->X_hat_Last + _First_Order_Kalman_Filter->Kalman_Gain * (_Z - _First_Order_Kalman_Filter->X_hat_Last);
    // Step3:预测误差更新
    _First_Order_Kalman_Filter->Error_Est = (1 - _First_Order_Kalman_Filter->Kalman_Gain) * _First_Order_Kalman_Filter->Error_Est_Last;
...
void data_upload_handle(uint16_t cmd_id, uint8_t *p_data, uint16_t len, uint8_t sof, uint8_t *tx_buf)
{
  ...
  // write_uart_blocking(&COMPUTER_HUART, tx_buf, frame_length);
  ...
  else if (sof == DN_REG_ID)
	{
	  ...
	  //Uart5SendBytesInfoProc(tx_buf,frame_length);
		
	  usart5.Send_bytes(&usart5,tx_buf,frame_length);

		
	//	Uart2SendBytesInfoProc(tx_buf,frame_length);
...
{if(judge_rece_mesg.game_robot_state.robot_level<=2)
							{
									...
									{ 
											b_chassis.max_speed = 2.33;
									}
									else
											b_chassis.max_speed = 1.9;  // 最大速度
							}
							else
							{
