...
void EncoderProcess(volatile Encoder *v, CanRxMsg * msg)
{
	...
	//转换为弧度制，REDUCTION_RATIO_13为电机减速比
    v->angle =( (float)(v->cal_data.raw_value - v->cal_data.ecd_bias) * 0.04394531f * PI / 180.0f + v->cal_data.round_cnt * 2 * PI)/(REDUCTION_RATIO_16);
	...
	//
	v->filter_rate = (int32_t)(temp_sum/RATE_BUF_SIZE)/REDUCTION_RATIO_16;		
	v->rate_rpm = (msg->Data[2]<<8)|msg->Data[3];//转子转速
	v->gyro=v->rate_rpm*2*PI/60/REDUCTION_RATIO_16;//转化为弧度制角速度
	v->current_raw = (msg->Data[4]<<8)|msg->Data[5];
	v->Torque=v->current_raw * 0.2462824;
}
