...
void USART5_DMA_R_T_JUDGE_Init(void)
{
  ...
  DMA_InitStructure.DMA_DIR 			    = DMA_DIR_MemoryToPeripheral;
	DMA_InitStructure.DMA_BufferSize			= sizeof(USART5_Tx_Buf);   //发送数据放在该数组中
	 ...
	 if(sof == DN_REG_ID 
			...
			&& Verify_CRC16_Check_Sum(&p_frame[deal_cnt], Frame_length))
		{
		    ...
		    case GAME_ROBOT_SURVIVORS_ID ://机器人存活数据：0x0003。发送频率：1Hz 
				...
				case REFEREE_WARNING_ID://场地事件数据：0x0101。发送频率：事件改变后发送
						memcpy(&judge_rece_mesg.referee_warning, data_addr, data_length);	
					break;
				
		    case DART_LAUNCH_ID ://飞镖发射相关数据：0x0105。发送频率：1Hz
				...
				case BUFF_MUSK_ENERGY_ID://机器人增益：0x0204。发送频率：状态改变后发送 
				...
				case PROJECTILE_ALLOWANCE_ID://0x208
					{
						int16_t	static bullet_supply_num=0,already_shoot=0,this_remain=0,last_remain=0,\
							ext_bullet_remaining=0,bullet_supply_flag=0;
				...
				case ROBOT_CUSTOM_CONTROLLER_DATA_ID://自定义控制器：0x0302.上限 30hz
