...
float pid_calc_angle_dir(pid_t *pid,float get,float set,float dir)
{
  pid->angle_dir=dir;
  pid->get = get;
  pid->set = set;
  float err=set - get;
  // wrap 到 [0, 2π)
        while (err >= TWO_PI) err -= TWO_PI;
        while (err <    0.0f) err += TWO_PI;
        if (pid->angle_dir == 0) // 顺时针 CW
        {
            if (err < 0.0f)
                err += TWO_PI;
        }
        else if (pid->angle_dir == 1) // 逆时针 CCW
        {
            if (err > 0.0f)
                err -= TWO_PI;
        }
		pid->err[NOW] = err;		
		 // 输入过大，直接返回 0（防抖/保护）
    if ((pid->input_max_err != 0.0f) && (fabsf(pid->err[NOW]) > pid->input_max_err))
        return 0;
	
	if (pid->pid_mode == POSITION_PID) // 位置式 PID
    {
        pid->pout = pid->p * pid->err[NOW];
        pid->iout += pid->i * pid->err[NOW];
        pid->dout = pid->d * (pid->err[NOW] - pid->err[LAST]);
    ...
    else if (pid->pid_mode == DELTA_PID) // 增量式 PID
    {
        pid->pout = pid->p * (pid->err[NOW] - pid->err[LAST]);
        pid->iout = pid->i * pid->err[NOW];
    ...
    // 输出死区
    if ((pid->output_deadband != 0.0f) && (fabsf(pid->out) < pid->output_deadband))
        return 0;
    else
        return pid->out;
